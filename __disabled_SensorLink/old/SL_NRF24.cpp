...
SL_NRF24::SL_NRF24(uint8_t chipEnablePin, uint8_t slaveSelectPin)
  : radio(chipEnablePin, slaveSelectPin)
{
} 

bool SL_NRF24::config()
{
  //Serial.println("Iniciando SL_NRF24");
  radio.begin();
  radio.setAutoAck(1);             // AutoACK is enabled
  radio.enableAckPayload();        // Allow optional ack payloads
  radio.setRetries(0,15);          // Time between retries, max no. of retries
...
uint8_t SL_NRF24::findChannel()
{
  uint8_t i;
  //Serial.println("Find channel SL_NRF24"); 
  radio.begin();
  radio.setAutoAck(false);
  for (i=FIRST_CHANNEL; i<=LAST_CHANNEL; i=i+CHANNEL_SPACE){
     radio.setChannel(i);
     if (!radio.testCarrier()) 
	 { 
      //  Serial.println("Found free channel:",i);
        if (i<255) return i;              // return free channel
 	 }
    return 255;                          // no channel avaiable
  }
}
...
bool SL_NRF24::send()
{
	...
	//Serial.println("Enviando SL_NRF24");
